...
std::string Hashing::generate_sha256(const std::string& input) {
    // Initial SHA-256 hash constants
    ...
    // Padding the byte array to match block requirements
    msg.push_back(0x80);
    ...
    // Process message block-by-block (64-byte chunks)
    ...
    // Convert bits to final 64-character hex string output
    std::stringstream ss;
